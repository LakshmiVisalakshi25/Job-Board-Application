  ...
  const toggleBookmark = async () => {
    try {
      if (isBookmarked) {
        ...
        setBookmarkedJobs(bookmarkedJobs.filter((id) => id !== job._id));
      } else {
        await fetch("https://job-board-backend-755o.onrender.com/api/bookmarks", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        ...
        <div className="bg-white dark:bg-gray-900 rounded-3xl border border-indigo-100 dark:border-gray-800 shadow-xl overflow-hidden">
          ...
          <div className="bg-gradient-to-r from-indigo-600 to-purple-600 px-6 py-8">
            <div className="flex items-start justify-between gap-4">
              <div>
                <h1 className="text-2xl sm:text-3xl font-bold text-white leading-snug">{job.title}</h1>
                <div className="flex flex-wrap items-center gap-3 mt-2 text-indigo-200 text-sm">
                  <span>📍 {job.location}</span>
                  <span className="text-white font-semibold">₹{job.salary?.toLocaleString()}</span>
                </div>
              </div>
              {job.type && (
          ...
          <div className="px-6 py-6 space-y-6">
            ...
            {job.skills?.length > 0 && (
              <div>
                <h2 className="text-sm font-semibold text-slate-500 dark:text-gray-400 uppercase tracking-wide mb-2">Skills Required</h2>
            ...
            {job.description && (
              <div>
                <h2 className="text-sm font-semibold text-slate-500 dark:text-gray-400 uppercase tracking-wide mb-2">About this Role</h2>
                <p className="text-sm text-slate-600 dark:text-gray-300 leading-relaxed whitespace-pre-line">{job.description}</p>
              </div>
            )}
        ...
        >
          <div className="bg-white dark:bg-gray-900 w-full max-w-md rounded-3xl shadow-2xl border border-indigo-100 overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4 border-b border-indigo-100 dark:border-gray-800">
              ...
              <button onClick={() => setShowForm(false)} className="w-8 h-8 rounded-lg flex items-center justify-center text-slate-400 hover:bg-slate-100 dark:hover:bg-gray-800">✕</button>
            </div>
            ...
            <div className="flex gap-3 px-6 py-4 border-t border-indigo-100 dark:border-gray-800">
              <button onClick={() => setShowForm(false)} className="flex-1 py-2.5 text-sm rounded-xl font-medium bg-slate-100 dark:bg-gray-800 text-slate-600 dark:text-slate-300 hover:bg-slate-200 transition-colors">Cancel</button>
              <button onClick={submitApplication} className="flex-1 py-2.5 text-sm rounded-xl font-medium bg-gradient-to-r from-indigo-600 to-purple-600 hover:opacity-90 text-white transition-all">Submit Application</button>
            </div>
          </div>
        </div>
      )}
